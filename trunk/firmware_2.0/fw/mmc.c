...
unsigned char mmc_write_stream_write_partial(unsigned char *buffer,unsigned long size,unsigned long *written,unsigned char *blockcompleted,unsigned long *currentaddr)
{
	...
	*blockcompleted=0;
	if(currentaddr)
		*currentaddr = _mmc_write_stream_address;
	...
	*written = effw;
	...
	if(_mmc_write_stream_numwritten>=512)
	{
		...
		*blockcompleted=1;
		...
		// Update the write address for the next write call.
...
unsigned char _mmc_write_multiblock_open(unsigned long addr)
{
	...
	#ifdef MMCDBG
		printf_P(PSTR("_mmc_write_multiblock_open\r"));
	#endif	
...
unsigned char _mmc_write_multiblock_close(void)
{
	...
	#ifdef MMCDBG
		printf_P(PSTR("_mmc_write_multiblock_close\r"));
	#endif
...
*******************************************************************************
	...
	Writes the lowest number of bytes between size, and the remaining space in the block. Returns the number of bytes effectively written.
		...
		currentaddr	address of the block currently being used to store data. If 0, the address will not be provided
	...
	In case of error in write_partial the following occurs:
		- the multiple block write is terminated (application needs not call write_close - this is internally done)
		- the stream is in closed state - attempt to open it will be done the next time mmc_write_streammulti_write_partial is called.
...
unsigned char mmc_write_streammulti_write_partial(unsigned char *buffer, unsigned long size,unsigned long *written,unsigned char *blockcompleted,unsigned long *currentaddr)
{
	...
	#ifdef MMCDBG
		//printf_P(PSTR("mmc_write_streammulti_write_partial. size: %d. strmopen: %d strmstrt: %d numwritten: %ld addr: %lX\r"),_mmc_write_stream_open,_mmc_write_stream_started,_mmc_write_stream_numwritten,_mmc_write_stream_address);
	#endif
	...
	*blockcompleted=0;
	...
	if(!_mmc_write_stream_open)
	{
		...
		{
			#ifdef MMCDBG
				printf_P(PSTR("mmc_write_streammulti_write_partial. size: %ld. strmopen: %d strmstrt: %d numwritten: %ld addr: %lX\r"),size,_mmc_write_stream_open,_mmc_write_stream_started,_mmc_write_stream_numwritten,_mmc_write_stream_address);
				printf_P(PSTR("mmc_write_streammulti_write_partial. _mmc_write_stream_address failed\r"));
			#endif
	...
	*written = effw;
	...
	if(_mmc_write_stream_numwritten>=512)
	{
		...
		*blockcompleted=1;
		...
		_mmc_write_stream_started=0;								// Block hasn't started
		...
		{
			_mmc_write_multiblock_close();
			_mmc_write_stream_open=0;							// Multiblock write not open
			return 2;
		}
...
unsigned char mmc_write_streammulti_close(unsigned char padchar,unsigned long *currentaddr)
{
	...
	#ifdef MMCDBG
		printf_P(PSTR("mmc_write_streammulti_close. strmopen: %d strmstrt: %d numwritten: %ld addr: %lX\r"),_mmc_write_stream_open,_mmc_write_stream_started,_mmc_write_stream_numwritten,_mmc_write_stream_address);
	#endif
	...
	if(_mmc_write_stream_started)
	{
		...
		// Stop the block
		response = _mmc_write_block_stop();
	}

	// Now terminates the multiple block write operation
	...
	response = _mmc_write_multiblock_close();
